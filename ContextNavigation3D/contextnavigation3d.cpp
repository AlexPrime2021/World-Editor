...
void ContextNavigation3D::cameraPan(const QVector2D &mousePosition)
{
    ...
    m_currentTranslation += QVector2D(diff.x() / m_speedPan, -diff.y() / m_speedPan);

    m_camera->setPosition(QVector3D(m_currentTranslation.x(), m_currentTranslation.y(), m_camera->position().z()));
    m_viewport->update();
}
